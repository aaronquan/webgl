...
export class WallEngine extends App.BaseEngine{
  ...
  hover_grid_side: Consts.GridCellSection | undefined;
  ...
  sideOnGrid(pos: WebGL.Matrix.Point2D | undefined): Consts.GridCellSection | undefined{
    if(pos != undefined){
      const dx = pos.x % 1;
      const dy = pos.y % 1;
      ...
      if(NumberUtils.distanceSq(dx, dy, 0.5, 0.5) < dist){
        this.hover_grid_side = Consts.GridCellSectionEnum.Center;
      }else{
  ...
  protected override handleMouseDown(ev: MouseEvent){
    if(this.mouse_over_cell != undefined){
     const cell = this.mouse_over_cell;
      ...
      //node selection
      else if(this.edit_state === Consts.WallEditStateEnum.Selecting){
      ...
      else if(this.edit_state === Consts.WallEditStateEnum.Adding){
      ...
      }else if(this.edit_state === Consts.WallEditStateEnum.Deleting){
        if(this.hover_grid_side != undefined){
  ...
  editSide(cell: Grid.GridPosition, side: Consts.GridCellSection, value: Grid.TileState): boolean{
    const side_to_direction = {
      [Consts.GridCellSectionEnum.Left]: Grid.DirectionEnum.Left,
      [Consts.GridCellSectionEnum.Down]: Grid.DirectionEnum.Down,
      [Consts.GridCellSectionEnum.Right]: Grid.DirectionEnum.Right,
      [Consts.GridCellSectionEnum.Up]: Grid.DirectionEnum.Up
    }
...
export class WallRenderer implements App.IEngineRenderer<WallEngine>{
  ...
  render(engine: WallEngine){
    ...
    if(engine.hover_grid_side != undefined){
      const text = engine.hover_grid_side === Consts.GridCellSectionEnum.Center ? "Center" 
